    ...
    <v-container >
            ...
            <v-card>
                <v-card-title>
                    ...
                    <span class="headline">Edit <span style="display:inline:color:green"> {{mix.name}} </span></span>
                    </v-card-title>
                    <v-divider></v-divider>
    ...
    methods:{
        ...
        async editMix(){
            ...
            if(this.$refs.form.validate() && this.chips.length > 0){
                ...
                await axios.put('/mix/'+this.mix.id,{
                    name:this.name,
                    discount:this.discount,
                    food:this.food,
                    availability:this.availability
                }).then(async (response)=>{
                    await this.$store.dispatch("editMix", response.data)
